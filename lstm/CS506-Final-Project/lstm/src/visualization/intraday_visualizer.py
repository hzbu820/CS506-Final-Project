...
def visualize_intraday_data(data, ticker, interval, output_name=None):
    ...
    # Hour vs Avg Price
    ax1.bar(hourly_patterns['Hour'], hourly_patterns['Close'], color='blue', alpha=0.7)
    ax1.set_title('Average Price by Hour', fontsize=14)
    ax1.set_xlabel('Hour')
    ...
    # Hour vs Avg Volume
    ax2.bar(hourly_patterns['Hour'], hourly_patterns['Volume'], color='green', alpha=0.7)
    ax2.set_title('Average Volume by Hour', fontsize=14)
    ax2.set_xlabel('Hour')
    ax2.set_ylabel('Avg Volume')
    ax2.set_xticks(range(24))
    ax2.grid(True, alpha=0.3)
    
    # Hour vs Percent Change
    ax3.bar(hourly_patterns['Hour'], hourly_patterns['Percent_Change'], color='red', alpha=0.7)
    ax3.set_title('Average % Change by Hour', fontsize=14)
    ax3.set_xlabel('Hour')
    ax3.set_ylabel('Avg % Change')
    ax3.set_xticks(range(24))
    ax3.grid(True, alpha=0.3)
    ax3.axhline(y=0, color='black', linestyle='-', linewidth=0.5)
    
    # Hour vs RSI
    ax4.bar(hourly_patterns['Hour'], hourly_patterns['RSI'], color='purple', alpha=0.7)
    ax4.set_title('Average RSI by Hour', fontsize=14)
    ax4.set_xlabel('Hour')
    ax4.set_ylabel('Avg RSI')
    ...
    print(f"Visualization saved to outputs/figures/{base_filename}_*.png")

def main():
    """Main function to run the intraday data visualizer"""
    # Parse arguments
    ...
    try:
        ...
        print("Intraday visualization completed successfully.")
        
    except Exception as e:
        print(f"Error in intraday visualization: {e}")
        import traceback
        traceback.print_exc()
